...
fn test_playing_state_cursor_movement() {
    ...
    assert_eq!(session.ui_state.cursor_pos, (initial_pos.0, initial_pos.1 + 1));
    
    state.handle_input(&mut session, KeyEvent::from(KeyCode::Down));
    assert_eq!(session.ui_state.cursor_pos, (initial_pos.0 + 1, initial_pos.1 + 1));
    
    state.handle_input(&mut session, KeyEvent::from(KeyCode::Left));
    assert_eq!(session.ui_state.cursor_pos, (initial_pos.0 + 1, initial_pos.1));
