...
fn test_two_player_mode_alternates_turns() {
    ...
    assert_eq!(session.game.current_player, PieceColor::White);
    ...
    session.select_piece(2, 1).unwrap();
    session.make_move(3, 0).unwrap();
...
fn test_both_players_can_move_pieces() {
    ...
    session.select_piece(2, 7).unwrap();
    session.make_move(3, 6).unwrap();

    // White moves again
    session.select_piece(5, 2).unwrap();
    session.make_move(4, 3).unwrap();

    // Black moves again - avoiding capture
    session.select_piece(2, 5).unwrap();
    session.make_move(3, 4).unwrap();

    // Verify pieces moved correctly
    assert!(session.game.board.get_piece(4, 1).is_some());
    assert!(session.game.board.get_piece(3, 6).is_some());
    assert!(session.game.board.get_piece(4, 3).is_some());
    assert!(session.game.board.get_piece(3, 4).is_some());
}
...
fn test_both_players_can_capture() {
    ...
    session.select_piece(2, 3).unwrap();
    session.make_move(3, 2).unwrap();

    // Now White must capture
    session.select_piece(4, 1).unwrap();
    session.make_move(2, 3).unwrap();

    // Verify capture
    assert!(session.game.board.get_piece(3, 2).is_none()); // Black piece removed
    assert!(session.game.board.get_piece(2, 3).is_some()); // White piece at destination
    ...
    if black_has_capture {
        ...
        if session.game.board.get_piece(1, 2).is_some() {
            session.select_piece(1, 2).unwrap();
            session.make_move(3, 4).unwrap();
        } else if session.game.board.get_piece(1, 4).is_some() {
            session.select_piece(1, 4).unwrap();
            session.make_move(3, 2).unwrap();
        }
    } else {
...
fn test_game_continues_without_ai() {
    ...
    // White move 3
    session.select_piece(5, 4).unwrap();
    session.make_move(4, 5).unwrap();
    assert_eq!(session.game.current_player, PieceColor::Black);
}
