    ...
    fn handle_input(&mut self, session: &mut GameSession, _key: KeyEvent) -> StateTransition {
        ...
        if !self.move_requested {
            ...
            if use_real_ai {
                        ...
                        match session.game.make_move(from_row, from_col, to_row, to_col) {
                            Ok(_) => {
                                ...
                                if let Some(ref provider) = session.hint_provider {
                                    if session.game.current_player == Color::White && !session.game.is_game_over {
                                        let hint_result = tokio::task::block_in_place(|| {
                                            tokio::runtime::Handle::current().block_on(
                                                provider.get_hint(&session.game.board, Color::White, &session.game.move_history)
                                            )
                                        });
                                        
                                        match hint_result {
                                            Ok(hint_text) => {
                                                session.hint = Some(crate::ai::Hint { hint: hint_text });
                                            }
                                            Err(_) => {
                                                session.hint = None;
                                            }
                            ...
                            Err(e) => {
                                session.ai_state.set_error(format!("AI failed to move: {}", e));
                ...
                let all_moves = game_logic::get_all_valid_moves_for_player(&session.game.board, Color::Black);
                
                if all_moves.is_empty() {
                    // No valid moves - game over
                    session.game.is_game_over = true;
                    return StateTransition::To(Box::new(
                        super::GameOverState::new(Some(Color::White))
                    ));
                }
                
                // Separate captures from regular moves
                let captures: Vec<_> = all_moves.iter()
