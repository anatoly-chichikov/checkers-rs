use crate::ai::{Hint, hint::HintProvider};
...
impl GameSession {
    ...
    pub fn make_move(&mut self, to_row: usize, to_col: usize) -> Result<bool, GameError> {
        let (from_row, from_col) = self.ui_state.selected_piece.ok_or(GameError::NoPieceSelected)?;
        
        let continue_capture = self.game.make_move(from_row, from_col, to_row, to_col)?;
        
        if continue_capture {
            self.ui_state.select_piece((to_row, to_col), &self.game.board);
        } else {
            self.ui_state.clear_selection();
        }
